param(
    [string]$Hl2Dir = "C:\Program Files (x86)\Steam\steamapps\common\Half-Life 2",
    [string]$OutputDir = "./hl2_astc",
...
$Vtf2Tga = Join-Path $Hl2Dir "bin\vtf2tga.exe"
$Python = "python"
...
function Extract-VtfViaPython {
    param([string]$VpkDirPath, [string]$OutDir, [string]$VpkName = "hl2_textures_dir.vpk")
...
# Engine cubemap face order: RT, LF, BK, FT, UP, DN.
# NOTE: vtf2tga -i emits faces named rt/lf/up/dn/ft/bk plus an extra "sph"
# sphere-map file, so use "lf" (not "lt") and ignore the "sph" face.
$CUBEMAP_FACE_SUFFIXES = @("rt", "lf", "bk", "ft", "up", "dn")
...
$tmpDir = Join-Path $env:TEMP "hl2_vtf_$(Get-Random)"
$null = New-Item -ItemType Directory -Path $tmpDir -Force

$totalExtracted = 0
$vpkSearchDirs = @(
    @{ Dir = (Join-Path $Hl2Dir "hl2"); Label = "hl2" }
)
    ...
    $results = $todo | ForEach-Object -ThrottleLimit $Threads -Parallel {
        ...
        try {
            ...
            if ($isCubemap) {
                ...
                $faceSuffixes = @("rt", "lf", "bk", "ft", "up", "dn")
                $facePaths = @()
                $allFound = $true
...
$dirsToCopy = @("hl2", "platform", "steam_input")
foreach ($d in $dirsToCopy) {
    $src = Join-Path $Hl2Dir $d
